...
- при смене поколения моделей — перечитай шаблоны `agents/` с вопросом «какие инструкции компенсируют слабости прошлых моделей» (пошаговые рецепты, разжёванные детали): компенсирующий скаффолдинг выпиливай, инварианты оставляй; `model:` в шаблонах — `inherit`, конкретную модель поколения не приколачивать.
