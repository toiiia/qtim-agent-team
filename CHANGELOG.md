...
> team-sync: рекомендуется командам с PM-дорожкой — обновит шаблон роли `product` и read-on-spawn в charter под продуктовую память; dev-only команды работают без изменений. Перечень — `reference/migrations.md`, запись «→ 1.7.0».
