...
if [ -z "$PLUGIN_V" ] || [ "$CHARTER_V" = "$PLUGIN_V" ]; then
  ...
  echo "[qtim] Команда настроена, но собрана по версии плагина ${CHARTER_V:-«до 1.3.0, штампа нет»}, а установлена v$PLUGIN_V — сгенерированные файлы (charter/агенты/settings) могли разойтись с движком. Запусти /qtim:team-sync: миграция подтянет изменения, не трогая проектную конкретику и memory/. Работать можно и без этого: /qtim:feature | /qtim:team-up | /qtim:team-lazy | /qtim:team-down."
fi
