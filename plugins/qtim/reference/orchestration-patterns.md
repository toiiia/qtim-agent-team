...
- **`meta` — чистый литерал** (без переменных/вызовов). Первая строка скрипта.
...
- **Loop всегда с budget-guard** (`while (budget.total && budget.remaining() > N)`), иначе
  упрётся в 1000-агентный backstop.
- **Мутации в параллель → `isolation: 'worktree'`** (несколько агентов правят файлы). Для
  read-only оппонентов/судей не нужно.

**Named workflows (опционально):** проект может держать готовые параметризованные скрипты в
`workflows/` (например `ensemble-review` — мульти-линзовый pre-merge review + скептик-верификация
+ codex-голос на P0/P1, обязателен для money-critical эпиков перед APPROVED; и `rls-audit` /
audit-карта видимости — inventory → fan-out по сущностям → карта). Запуск:
`Workflow({ name: 'ensemble-review', args: {...} })` — opt-in правило в силе. Результат
коммитит в `memory/` оркестратор.
...
const winner = await agent(
  `Сравни ${drafts.filter(Boolean).length} ADR попарно. Рубрика: инварианты > идемпотентность
...
**Когда:** маршрутизация входящих. Это **автоматизация decision matrix** (A/B/C/D) и таблицы
`triggers` — агент-классификатор вместо keyword-match. Два сценария: триаж одного бага (лёгкий
classifier → team-lead роутит в Team) и разбор пачки (бэклог P1..P3, open-баги —
классифицировать по слою/severity/владельцу).
...
await pipeline(incomingBugs,
  bug => agent(`Классифицируй: слой (data/SQL→db | UI→front | infra→devops | sec→auditor),
                severity P0..P3, режим A/B/C/D. Сверься с triggers в charter.`, { schema: TRIAGE }),
  (t, bug) => t.severity === 'P0'
    ? agent(`Хотфикс ${bug.id}: ${bug.title}`,
            { agentType: t.owner === 'db' ? 'database-agent' : 'frontend-agent' })
...
const map = await agent(
  `Собери карту видимости по ролям из находок. Где щели на стыках дочерних сущностей?`,
...
```js
const m = await agent('Security-critical миграция <описание>', { agentType: 'database-agent' })
...
const top = await agent(
  `Отфильтруй до 5. Рубрика: тон (деловой, не казённый) > длина ≤80 символов > ясность
   > без англицизмов > грамматика целевой локали. Верни топ-5 с обоснованием.`, { schema: RANKED })
...
`../commands/team-up.md` отвечает на «**сколько** ролей / какой объём» (A/B/C/D). Этот файл —
ортогональное измерение «**какая форма** оркестрации». Сначала объём (A/B/C/D), затем —
нужен ли паттерн отсюда. Большинство задач паттерна не требуют: штатный Team-флоу остаётся
дефолтом, паттерны — для задач с явной структурой (выбор/сходимость/широкий разбор/состязание).
