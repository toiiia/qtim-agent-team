...
инварианты, реестр решений; продуктовую память, если создана (`memory/product-map.md`,
`product-actors.md`, `product-glossary.md`, `product-metrics.md` — её наполняет
`/qtim:product-onboard`). Если работаешь по фиче — её артефакты в `docs/features/<slug>/`.
