...
4. Если `docs/features/<slug>/` уже существует — прочитай Status артефактов и **продолжи с первой
   стадии, чей артефакт не Approved**. С нуля не перезапускай.

## Стадии

Checkpoint после каждой стадии — через `AskUserQuestion` (или прямой вопрос): без подтверждения
на следующую стадию не переходишь. Интенсивность подтверждений соответствует Intake-режиму charter.
...
questions. **Checkpoint:** пользователь утверждает или правит; Status → Approved.
...
1. Добавь строку-указатель на фичу в реестр решений `memory/`.
