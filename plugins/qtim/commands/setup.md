...
- **Файлы памяти** — структура `memory/`. При PM-дорожке пометь, что реестр решений хранит строки-указатели на фичи в `docs/features/<slug>/` (сами артефакты в memory не дублируются; директорию создаёт `/qtim:feature` per-slug, setup её не создаёт).
