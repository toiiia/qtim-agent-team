...
### Skills под стек
Просканируй skills, доступные в текущей сессии (они перечислены в системном контексте), при необходимости уточни через `find-skills`. Сопоставь по смыслу с обнаруженным стеком. Ориентир-эвристика (адаптируй под реально доступные — набор зависит от окружения):
...
| Все роли (universal) | `using-superpowers`, `karpathy-guidelines`; `brainstorming` / `grill-me` — architect |

Каждый кандидат привяжи к роли (в чьём `skills`-поле он окажется).

### Плагины / MCP под стек
Сопоставь стек с маркетплейсом. Источник — официальный каталог (`/plugin marketplace`) и реестр MCP (`search_mcp_registry` / `suggest_connectors` из mcp-registry, если доступны). Известные сопоставления:
...
Впиши подтверждённые (Q8) skills в `skills`-поле каждой роли — и в charter-таблицу (4.1), и в напоминание промпта при спавне (universal skills всегда + подобранные специальные под роль). Skill попадает в ту роль, к которой привязан в Phase 1b.
