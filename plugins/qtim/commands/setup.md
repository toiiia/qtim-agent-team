...
> **Что генерируешь vs что даёт плагин.** Команды движка (`team-up`/`team-lazy`/`team-down`/`team-sync`/`team-retro`/`onboard`/`doctor`/`feature`) доступны как slash-команды `/qtim:*`, переносимая механика (`reference/intake-protocol.md`, `orchestration-patterns.md`, `codex-consult.md`, `feature-pipeline.md`, `migrations.md`) живёт в `${CLAUDE_PLUGIN_ROOT}/reference/` как supporting-docs (НЕ slash-команды), готовые Workflow-скрипты — в `${CLAUDE_PLUGIN_ROOT}/workflows/`. И то и другое — **не копируешь**. Ты генерируешь только проект-специфичное: `team-charter.md`, определения субагентов под стек, baseline-hooks и память. Шаблоны ролей бери из `${CLAUDE_PLUGIN_ROOT}/agents/*.md` и подставляй плейсхолдеры под выявленный стек.
...
### Skills под стек — источники (по возрастанию стоимости)
Подбирай в этом порядке, деградируя gracefully если канал недоступен:

0. **Собственные скилы плагина qtim** (`qtim:debug-loop`, `qtim:prototype`) — приходят с плагином, есть в любой сессии с установленным qtim. В `skills`-поле ролей их **не вписывай**: шаблоны ролей ссылаются на них напрямую. При Q6=Standalone они копируются в `.claude/skills/` и зовутся без префикса — см. 4.6.
...
| Все роли (universal) | `using-superpowers`, `karpathy-guidelines` — только если реально доступны в сессии (дисциплины разбора и стресс-теста architect'а закрыты слоем 0: `qtim:brainstorm`, `qtim:grill`) |
...
### Q0. Роль пользователя
Кто будет работать с командой в этом проекте (нумерация Q0 — чтобы не сдвигать устоявшиеся ссылки на Q1-Q8):
- **Developer (дефолт)** — команда разработки: текущий флоу, все вопросы ниже как есть.
- **PM/Analyst** — продуктовый трек: роль `product` и конвейер `/qtim:feature` (хотелка → PRD → декомпозиция → оценка → план) с артефактами в `docs/features/`. Q2 не задаётся — состав фиксируется стеком: `product` + `architect` + профильные impl/`tester` под стек (**без** `reviewer`); dev-роли нужны конвейеру как read-only консультанты для точной декомпозиции и оценки, даже если пользователь код не пишет. Charter получает секцию «PM-конвейер» с пометкой: PM-состав рассчитан на конвейер документов, не на реализацию — перед запуском handoff-плана в разработку (`/qtim:team-up`, режим D с петлёй через `reviewer`) состав дополняется dev-дорожкой повторным `/qtim:setup` (он дописывает, не пересоздаёт).
- **Оба** — обе дорожки: полный состав по Q2 + `product`, charter получает и dev-флоу, и секцию «PM-конвейер».

Если charter уже существует и содержит только другую дорожку — предложи **дописать недостающее** (секцию «PM-конвейер» и/или роль `product`), не пересоздавая существующее.
...
- Decision log — реестр решений и фич `memory/decisions.md` (рекомендую: его читают architect/product на спавне, пишут approval-гейт intake-протокола и handoff `/qtim:feature`)
...
- **No** — пропустить (можно добавить позже повторным `/qtim:setup`). Charter получит секцию «Codex second-opinion» из одной строки-заглушки «выключен (Q5=No)», а codex-блоки из шаблонов ролей в сгенерированные файлы не переносятся (см. 4.2) — роли не должны искать протокол, от которого пользователь отказался.
...
- **Standalone** — дополнительно скопировать `commands/*`, `reference/*`, `workflows/*` и `skills/*` в `.claude/` проекта. Самодостаточно (работает без плагина); обновления движка — через `/qtim:team-sync` при установленном плагине, иначе вручную.

### Q7. Hooks-гейты (multi-select)
- PostToolUse: после правок кода — напоминание/запуск `{{TYPECHECK_CMD}}` (не блокирующий, рекомендую)
- SubagentStop: напоминание проверить артефакты роли — **только при Q6=Standalone** (нужен на случай работы без плагина)
- SessionStart: анонс «команда настроена, /team-up» — **только при Q6=Standalone** (аналогично)

Честная оговорка для Standalone: hooks плагина гейтятся наличием charter, а не режимом — пока плагин установлен, сгенерированные SessionStart/SubagentStop дублируют плагинные (двойной анонс/напоминание); дубль уходит после удаления плагина, `/qtim:doctor` его подсвечивает. В Plugin-linked эти два hook'а не генерируй вовсе — их даёт плагин.
...
- Шапка: версия charter (1.0) + дата + штамп `generated-by: qtim v<версия из ${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json> · mode: <plugin-linked|standalone>` (по Q6). По штампу SessionStart-hook детектит дрейф версий, а `/qtim:team-sync` определяет диапазон миграций — формат строки не менять.
...
- **Codex second-opinion** — при Q5=Yes: gate-точки + путь к протоколу `codex-consult.md`. **Путь записывай абсолютным** — тем реальным каталогом `reference/` плагина, который ты видишь в этом файле: плейсхолдер плагин-рута в сгенерированных файлах не резолвится, а субагенты читают протокол именно по пути из charter. При Q6=Standalone путь тоже абсолютный, но к локальной копии `.claude/reference/codex-consult.md`. Пометь рядом: путь machine-specific — на другой машине его чинит `/qtim:doctor` (п. 8) или `/qtim:team-sync` (проверяет резолвинг даже при совпадении версий). При Q5=No: секция из одной строки «выключен (Q5=No); включить — повторный `/qtim:setup`» — по ней team-up и роли понимают, что consult не настроен.
...
- **PM-конвейер** (если Q0 = PM/Analyst или Оба) — суть `reference/feature-pipeline.md`: два трека — полный (intake → PRD → декомпозиция → оценка → план → handoff; checkpoint'ы по стадиям, у декомпозиции и оценки общий) и fast-path для S/M-хотелок в одну фазу без развилок (один `feature-brief.md` с единственным checkpoint); схема `docs/features/<slug>/` (intake/prd/decomposition/estimate/plan или feature-brief, статусы Draft → Approved → In Development → Done); правило dev-consult на декомпозиции/оценке (consult профильных ролей read-only, состав ∝ реально задетым слоям, оценки S/M/L/XL только с evidence, размер даёт владелец слоя); правило нарезки (work item и фаза плана — вертикальный срез; широкий механический рефактор — expand–contract); handoff-контракт. Путь к полному протоколу — **абсолютный**, как у codex-consult: реальный каталог `reference/` плагина, при Standalone — локальная копия `.claude/reference/feature-pipeline.md`.
- **Файлы памяти** — структура `memory/` (по Q4; реестр решений и фич — каноническое имя `memory/decisions.md`). Перечисляй и файлы, которые роли создадут по мере работы (test-cases, bug-log, review-report, ui-spec, production-checklist…), с пометкой «создаётся ролью» — всё, на что ссылается `read on spawn` состава, должно быть объяснено здесь. При PM-дорожке пометь, что `memory/decisions.md` хранит строки-указатели на фичи в `docs/features/<slug>/` (сами артефакты в memory не дублируются; директорию создаёт `/qtim:feature` per-slug, setup её не создаёт). Продуктовую память (`memory/product-map.md`, `product-actors.md`, `product-glossary.md`, `product-metrics.md`) setup тоже не создаёт — её наполняет `/qtim:product-onboard`; впиши эти файлы в `read on spawn` роли `product` с пометкой «если созданы».

### 4.2 Агенты — `.claude/agents/<role>-agent.md`
Для каждой impl/специализированной роли из Q2 (или PM-состава по Q0): возьми шаблон из `${CLAUDE_PLUGIN_ROOT}/agents/<role>-agent.md`, **подставь плейсхолдеры** (Phase 1) на реальные команды/фреймворки, конкретизируй доменные инварианты из charter. Header-note шаблона (цитата «Это generic-шаблон роли…») в сгенерированный файл не переноси — она относится к шаблону, не к проекту. Сохрани frontmatter (`name`, `description` с example-блоками, `model`, `color`, `tools`, `memory`). `explorer` → `subagent_type: Explore` (свой файл обычно не нужен). `devops`/`auditor` → `general-purpose`, если не нужен выделенный тип.

Соответствие шаблонов: `architect-agent`, `database-agent`, `frontend-agent`, `testing-agent`, `reviewer-agent`, `product-agent` (роль `product` — PM-конвейер + UX-аудит; генерируй при Q0 = PM/Analyst / Оба или Extended-составе). Под другой стек переименуй impl-роль (`backend-agent`/`analytics-agent`), но сохрани каркас шаблона.
...
**Codex-блоки шаблонов** (цитаты «Codex second-opinion…» у db/reviewer/architect/product, включая упоминание codex в `description` frontmatter) переноси только при Q5=Yes; при Q5=No вырезай целиком — как стек-условные.

**Model по ролям:** шаблонное `model: inherit` — дефолт: роли наследуют модель сессии и апгрейдятся вместе с ней, конкретную модель поколения не приколачиваем. Исключение по умолчанию — токен-ёмкие механические роли: `tester` генерируй с явным дешёвым тиром (`sonnet`, при жёстком бюджете `haiku`), `explorer` — `haiku`; прогон сценариев и поиск по репо не окупают топ-модель. Интеллект-ёмкие роли (architect / reviewer / db / product / front) оставляй на inherit. Все отступления от inherit фиксируй в charter-таблице состава; у ролей без собственного файла (`explorer`) charter — единственное место фиксации, team-up передаёт тир при спавне.
...
У `product` каркаса больше нет — используй шаблон `product-agent.md` (PM-конвейер + прежний UX-аудит как режим UX-AUDIT).
...
`MEMORY.md` (индекс) + выбранные файлы (project map / commands / safety / invariants / decisions). Каждый — frontmatter `name`/`description`/`metadata.type`. `memory/decisions.md` — реестр решений и фич: заголовок + формат строки `дата · решение или фича · указатель (ADR / docs/features/<slug>/ / file:line)`, содержимое документов не дублируется.

### 4.5 CLAUDE.md
Если есть — добавь короткую секцию «Команда агентов» (указатель на `.claude/team-charter.md` + `/qtim:team-up` / `/qtim:team-lazy`; при PM-дорожке — также `/qtim:feature` и конвенция `docs/features/<slug>/`). Если нет — предложи создать. Это единственный авто-загружаемый файл, без указателя из него команда не обнаружима в direct-сессии.

### 4.6 Standalone (только если Q6=Standalone)
Скопируй `${CLAUDE_PLUGIN_ROOT}/commands/{team-up,team-lazy,team-down,team-sync,team-retro,onboard,product-onboard,doctor,feature}.md` → `.claude/commands/`, `${CLAUDE_PLUGIN_ROOT}/reference/*` → `.claude/reference/`, `${CLAUDE_PLUGIN_ROOT}/workflows/*` → `.claude/workflows/` (там они доступны и как `Workflow({ name: … })`) и `${CLAUDE_PLUGIN_ROOT}/skills/*` → `.claude/skills/` (каталоги скилов целиком), поправив относительные ссылки. Командные имена локализуй **и в скопированных движковых файлах, и в сгенерированных** (charter, hooks, агенты): везде `/qtim:<cmd>` → `/<cmd>` — в том числе handoff-шаблон в `feature.md` (иначе он запишет в `plan.md` проекта несуществующую команду) и кросс-ссылки команд друг на друга; упоминания скилов `qtim:<skill>` → `<skill>` (скопированные в `.claude/skills/` они доступны без префикса плагина, а `qtim:*` без плагина не резолвится); проверь остатки `grep -rn 'qtim:' .claude/commands/ .claude/reference/ .claude/workflows/ .claude/skills/ .claude/agents/`. Исключение — упоминания `/qtim:setup`: он в standalone-набор не входит и всегда требует установленного плагина, оставляй имя как есть. Путь к протоколу codex-consult — абсолютный, на локальную копию `.claude/reference/codex-consult.md`.

---

## Phase 5: Verification & Handoff

Первым пунктом проверь активацию рантайма: если в **текущей** сессии нет инструментов Agent Teams (`SendMessage` / `TaskCreate`) — флаг из 4.3 применится только со следующего старта; явно предупреди, что перед первым `/qtim:team-up`/`/qtim:team-lazy` нужен перезапуск CLI.

Покажи пользователю: что создано (по разделам), как пользоваться (`/qtim:feature` для хотелки — если сгенерирована PM-дорожка, `/qtim:team-up` для эпика, `/qtim:team-lazy` для лёгких задач, `/qtim:team-down` для сворачивания, `/qtim:team-sync` после обновления плагина — SessionStart-hook сам подскажет при дрейфе версий), и действия от него (если Codex Yes — проверить `codex --version`; запустить команду на маленькой задаче). На существующей кодовой базе порекомендуй `/qtim:onboard` — глубокое наполнение памяти, а при PM-дорожке ещё `/qtim:product-onboard` — продуктовую память (разделы, акторы, словарь, аналитика), без которой intake/PRD опираются только на ответы пользователя; при проблемах — `/qtim:doctor`. Напомни: содержимое агентов и charter — обычный markdown, правится вручную; при правках держи charter источником истины.
