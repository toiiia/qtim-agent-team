...
Скопируй `${CLAUDE_PLUGIN_ROOT}/commands/{team-up,team-lazy,team-down,team-sync}.md` → `.claude/commands/` и `${CLAUDE_PLUGIN_ROOT}/reference/*` → `.claude/reference/`, поправив относительные ссылки. В сгенерированных файлах (charter, hooks) команды указывай локальными именами (`/team-up` вместо `/qtim:team-up`), а путь к протоколу codex-consult — абсолютным, на локальную копию `.claude/reference/codex-consult.md`.
