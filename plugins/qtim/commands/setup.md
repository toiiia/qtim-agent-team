...
- **Standalone** — дополнительно скопировать `commands/*` и `reference/*` в `.claude/` проекта. Самодостаточно (работает без плагина); обновления движка — через `/qtim:team-sync` при установленном плагине, иначе вручную.
