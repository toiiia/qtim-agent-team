...
- **Standalone** — дополнительно скопировать `commands/*`, `reference/*` и `workflows/*` в `.claude/` проекта. Самодостаточно (работает без плагина); обновления движка — через `/qtim:team-sync` при установленном плагине, иначе вручную.
...
- **PM-конвейер** (если Q0 = PM/Analyst или Оба) — суть `reference/feature-pipeline.md`: стадии intake → PRD → декомпозиция → оценка → план → handoff с checkpoint у пользователя; схема `docs/features/<slug>/` (intake/prd/decomposition/estimate/plan, статусы Draft → Approved → In Development → Done); правило dev-consult на декомпозиции/оценке (consult профильных ролей read-only, оценки S/M/L/XL только с evidence, размер даёт владелец слоя); handoff-контракт. Путь к полному протоколу — **абсолютный**, как у codex-consult: реальный каталог `reference/` плагина, при Standalone — локальная копия `.claude/reference/feature-pipeline.md`.
...
Скопируй `${CLAUDE_PLUGIN_ROOT}/commands/{team-up,team-lazy,team-down,team-sync,team-retro,onboard,product-onboard,doctor,feature}.md` → `.claude/commands/`, `${CLAUDE_PLUGIN_ROOT}/reference/*` → `.claude/reference/` и `${CLAUDE_PLUGIN_ROOT}/workflows/*` → `.claude/workflows/` (там они доступны и как `Workflow({ name: … })`), поправив относительные ссылки. Командные имена локализуй **и в скопированных движковых файлах, и в сгенерированных** (charter, hooks): везде `/qtim:<cmd>` → `/<cmd>` — в том числе handoff-шаблон в `feature.md` (иначе он запишет в `plan.md` проекта несуществующую команду) и кросс-ссылки команд друг на друга; проверь остатки `grep -rn '/qtim:' .claude/commands/ .claude/reference/ .claude/workflows/`. Исключение — упоминания `/qtim:setup`: он в standalone-набор не входит и всегда требует установленного плагина, оставляй имя как есть. Путь к протоколу codex-consult — абсолютный, на локальную копию `.claude/reference/codex-consult.md`.
